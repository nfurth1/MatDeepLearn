...
def trainer(
    ...
    train_plot, val_plot = np.array([]), np.array([])
    ...
    for epoch in range(1, epochs + 1):
        ...
        ##Print performance
        train_plot = np.append(train_plot, float(train_error))
        val_plot = np.append(val_plot, float(val_error))
    ...
    plot = np.array([train_plot, val_plot])
    dict = {'training': plot[0], 'val': plot[1]}
...
def write_plot(output, filename):
    ...
    with open(filename, "w") as f:
        ...
        for i in range(0, len(output)):
            if i == 0:
                csvwriter.writerow(
                    ["training error"] * int((shape[1] - 1) / 2)
                    + ["validation error"] * int((shape[1] - 1) / 2)
                )
            elif i > 0:
                csvwriter.writerow(output[i - 1, :])
